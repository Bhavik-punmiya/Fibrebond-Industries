...
const TableThree: React.FC = () => {
  ...
  const [selectedFamilyOptions, setSelectedFamilyOptions] = useState<FamilyOption[]>([]);

  const fetchFamilyNames = async (): Promise<void> => {
    try {
      const response = await axios.get('http://localhost:5000/api/v1/family/');
      const families: string[] = response.data; // This should be an array of family names
      const options: FamilyOption[] = families.map(familyName => ({
        value: familyName,
        label: familyName
      }));
  ...
  const handleFamilyChange = (selectedOptions: FamilyOption[]): void => {
    setSelectedFamilyOptions(selectedOptions);
  };

  const promiseOptions = (inputValue: string): Promise<FamilyOption[]> =>
    new Promise((resolve) => {
      setTimeout(() => {
              ...
              <tr key={key}>
                ...
                <td className="border-b border-[#eee] px-4 py-5 dark:border-strokedark">
                  <div className="flex items-center space-x-3.5">
                    <button className="hover:text-primary" onClick={() => openModal(user)}>
                      <svg
